...
import { cleanVerifyTitle, looksLikeInternalDesignedTitle } from './test-naming'
  ...
  return {
    ...
    steps: [
      ...
      makeStep(
        ...
        fillEls.length > 0
          ? fillEls.map((el) => `await ${pageVar}.${el.propertyName}.fill('test value')`)
          : [`await expect(${pageVar}.page.locator('body')).toBeVisible()`],
      ),
      makeStep('Submit or confirm', [
        submitEl
          ? `await ${pageVar}.${submitEl.propertyName}.click()`
          : `await expect(${pageVar}.page.locator('body')).toBeVisible()`,
      ]),
      makeStep('Assert expected outcome', [`await expect(${pageVar}.page).toHaveURL(/.+/, { timeout: 15_000 })`]),
    ],
  }
}
    ...
    steps: [
      {
        ...
        code: [
          ...
          `await expect(${pageVar}.page.locator('[data-test="inventory-container"], .inventory_list')).toBeVisible()`,
        ],
      },
    ],
  }
...
export function mergeDesignCasesIntoPlan(
  ...
  for (const designed of designedCases) {
    if (designed.level !== 'ui' && plan.pattern !== 'generic-form') {
