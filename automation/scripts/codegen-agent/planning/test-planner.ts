...
import {
  ...
  specExpectSearchResults,
  specExpectValidationError,
  specFillLocator,
...
function detectPattern(elements: ResolvedElement[]): PagePattern {
  ...
  const hasFillable = has(
    (e) =>
      e.uiAction === 'fillInput' ||
      e.uiAction === 'checkCheckbox' ||
      e.uiAction === 'selectOption' ||
      e.uiAction === 'uploadFile',
  )
  const hasSubmitButton = has((e) => e.uiAction === 'clickElement' && e.kind === 'button')
  ...
  if (!hasFillable && !hasSubmitButton) return 'interactive'
  return 'generic-form'
}
...
export class TestPlanner {
  generate(opts: {
    ...
    const testGroups = [
      ...this.buildGroups(pattern, opts.pageName, opts.url, opts.elements),
      ...this.overlayGroups(opts.pageName, opts.url, opts.elements),
    ]
  ...
  private buildGroups(
    ...
    switch (pattern) {
      ...
      case 'interactive':
        return this.interactiveGroups(pageName, url, elements)
      default:
        ...
        cases: [
          this.makeCase('mismatched passwords show confirm-password error', 'negative', page, [
            ...
            makeStep('Assert confirm-password validation error', [
              specExpectValidationError(page),
            ]),
          ]),
        ],
      },
  ...
  /**
   * Only reached when detectPattern() found real fillable inputs and a submit
   * button (see detectPattern's hasFillable/hasSubmitButton check) — otherwise
   * the page is routed to interactiveGroups() instead.
   */
        ...
        cases: [
          this.makeCase('successful form submission', 'happy-path', page, [
            ...
            makeStep(
              ...
              fillSteps.length > 0 ? fillSteps : [specExpectPageLoaded(page)],
            ),
            makeStep('Submit form', [
              submitEl
                ? specClickLocator(page, submitEl.propertyName)
                : specExpectPageLoaded(page),
            ]),
  ...
  /**
   * Pages with no fillable inputs and no submit button (detectPattern's
   * 'interactive' pattern) — e.g. plain content pages with links/buttons but
   * no form. Instead of fabricating a "form submission" case, verify the
   * actual observable effect of the primary interactive element(s).
   */
  private interactiveGroups(pageName: string, url: string, elements: ResolvedElement[]): TestGroup[] {
    const page = toPageVar(pageName)
    const linkCount = elements.filter((e) => e.kind === 'link').length
    const pathname = (() => {
      try {
    ...
    const isContentListing = linkCount >= 8 || pathname.includes('blog')

    const candidates = elements
      .filter((e) => e.kind === 'link' || e.kind === 'button')
      .filter((e) => !isDestructiveLabel(e.label || e.accessibleName || e.textContent || ''))
      .filter((e) => e.kind !== 'link' || classifyLinkTarget(e.href ?? '', url) !== 'external')
      .slice(0, 2)

    if (isContentListing || candidates.length === 0) {
      return [
        {
          groupName: 'Happy Path',
          requiresApiSetup: false,
          ...
          stateKey: '',
          cases: [
            this.makeCase('page loads with primary content', 'happy-path', page, [
              navigateStep(page, url),
              makeStep('Assert page is ready', [specExpectPageLoaded(page)]),
              makeStep('Assert URL is stable', [
                `await expect(${page}.page).toHaveURL(/.+/, { timeout: 15_000 })`,
              ]),
            ]),
            ...(linkCount > 0
              ? [
                  this.makeCase('navigation links are visible on the page', 'happy-path', page, [
                    navigateStep(page, url),
                    makeStep('Assert primary navigation links', [specExpectPageLoaded(page)]),
                  ]),
                ]
              : []),
          ],
    ...
    return [
      {
        ...
        apiEndpoint: '',
        stateKey: '',
        cases: candidates.map((el) => this.interactiveClickCase(page, url, el)),
      },
    ]
  }

  private interactiveClickCase(page: string, url: string, el: ResolvedElement): TestCase {
    const label = el.label || el.accessibleName || el.textContent || 'element'
    const target = classifyLinkTarget(el.href ?? '', url)

    if (el.kind === 'link' && target === 'internal-nav') {
      const destinationPath = (() => {
        try {
          return new URL(el.href ?? '', url).pathname
        } catch {
          return undefined
        }
      })()
      return this.makeCase(`clicking "${label}" navigates to the expected page`, 'happy-path', page, [
        navigateStep(page, url),
        makeStep(`Click "${label}"`, [specClickLocator(page, el.propertyName)]),
        makeStep('Assert navigation landed on the expected page', [
          destinationPath
            ? `await expect(${page}.page).toHaveURL(/${escapeRegExp(destinationPath)}/)`
            : `await expect(${page}.page).toHaveURL(/.+/, { timeout: 15_000 })`,
        ]),
      ])
    }

    return this.makeCase(`clicking "${label}" updates the page content`, 'happy-path', page, [
      navigateStep(page, url),
      // Capture, click and assert must share the "beforeText" local — keep them
      // in one test.step so it stays in the same closure scope.
      makeStep(`Click "${label}" and assert content changes`, [
        specCaptureText(page, 'beforeText'),
        specClickLocator(page, el.propertyName),
        specExpectContentChanged(page, 'beforeText'),
      ]),
    ])
  }
  ...
  private overlayGroups(pageName: string, url: string, elements: ResolvedElement[]): TestGroup[] {
    const overlayElements = elements.filter((el) => el.surfaceContext)
    if (overlayElements.length === 0) return []

    const page = toPageVar(pageName)
    const byContext = new Map<string, ResolvedElement[]>()

    for (const el of overlayElements) {
      const key = el.surfaceContext!
    ...
    for (const [context, contextElements] of byContext) {
      const primary =
        contextElements.find((el) => el.kind === 'button')
        ?? contextElements.find((el) => el.kind === 'input-text' || el.kind === 'input-email')
        ?? contextElements[0]

      if (!primary) continue

      const readableContext = context.replace(/([A-Z])/g, ' $1').trim().toLowerCase()
      cases.push(
        this.makeCase(
          `${readableContext} overlay exposes interactive controls`,
          'happy-path',
          page,
          [
            navigateStep(page, url),
            makeStep(`Assert ${readableContext} control is attached in the DOM`, [
              `await expect(${page}.${primary.propertyName}).toBeAttached()`,
            ]),
          ],
        ),
      )
